...
import { APP_NAME, APP_EMOJI, FEATURES, WEATHER_LAT, WEATHER_LON } from '../config';
...
export default function KioskView({ onAdminAccess }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${WEATHER_LAT}&longitude=${WEATHER_LON}&current_weather=true`)
      .then(r => r.json())
      .then(d => {
        if (d.current_weather) setWeather({
          temp: Math.round(d.current_weather.temperature),
          icon: WEATHER_ICONS[d.current_weather.weathercode] || '🌡️',
        });
      })
      .catch(() => {});
  }, []);
  ...
  const handleSelect = (child) => {
    setSelected(child); setWho(''); setSignature(''); setSigMode(false); setStep('action');
  };
  ...
  const handleAction = (type) => {
    ...
    else { setSigMode(true); setPinMode(type); setStep('signature'); }
  };
  ...
  const finalize = async (type, pin) => {
    ...
    try {
      if (type === 'in') {
        ...
        if (FEATURES.sounds) playCheckIn();
        if (FEATURES.confetti) launchConfetti();
        showToast(`✓ ${selected.name} checked in! Signed by ${who}`, 'grass');
      } else {
        await api.checkOut(selected.id, who);
        if (FEATURES.sounds) playCheckOut();
      ...
      setWho(''); setSignature(''); setSigMode(false); setPinMode(null);
    }
  };
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 720 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 52, marginBottom: 6, lineHeight: 1 }}>{APP_EMOJI}</div>
          <h1 style={{ fontSize: 34, color: 'var(--text)', fontWeight: 900, letterSpacing: -1 }}>
            {APP_NAME}
          </h1>
          <p style={{ color: 'var(--text2)', fontSize: 16, marginTop: 6 }}>
            {timeGreeting()}, {new Date().toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
            {presentCount > 0 && (
              <span className="count-pulse" style={{ marginLeft: 12, background: '#5BAD5B22', color: '#5BAD5B', padding: '2px 12px', borderRadius: 20, fontWeight: 700, fontSize: 14 }}>
                {presentCount} child{presentCount !== 1 ? 'ren' : ''} present
              </span>
            )}
          </p>
          {weather && (
            <div className="weather-pill" style={{ marginTop: 10 }}>
        ...
        {step === 'list' && (
          <>
            <div style={{ marginBottom: 16 }}>
              <input placeholder="🔍  Search child name..."
              ...
              {filtered.map(child => {
                ...
                return (
                  <button key={child.id} onClick={() => handleSelect(child)} className="hover-lift"
                    style={{
                      background: birthday ? 'linear-gradient(135deg, #FFF9E6, #fff)' : '#fff',
                      borderRadius: 18,
                  ...
                  >
                    <div className={birthday ? 'birthday-avatar' : ''}>
                      <Avatar child={child} size={52} />
                    </div>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 17, color: 'var(--text)' }}>{child.name}</div>
                      <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 6 }}>{displayAge}</div>
                      {birthday && (
                        <div className="birthday-badge" style={{ marginBottom: 4 }}>🎂 Birthday!</div>
                      )}
                      <div style={{
                        ...
                        {checked ? `✓ In since ${fmt(rec?.check_in)}` : 'Not checked in'}
                      </div>
                    </div>
                  </button>
                );
        ...
        {step === 'action' && selected && (
          <div className="slide-up" style={{
            ...
            <div className={isBirthdayToday(selected.dob) ? 'birthday-avatar' : ''}>
            ...
            {isBirthdayToday(selected.dob) && (
              <div className="birthday-badge" style={{ margin: '8px 0' }}>🎂 Happy Birthday!</div>
            )}
            {selected.notes && (
              <div style={{
            ...
            <div style={{ margin: '16px 0' }}>
              ...
              <input placeholder="Full name required..." value={who}
                onChange={e => setWho(e.target.value)}
                style={{ textAlign: 'center', fontSize: 16, borderRadius: 12, width: '100%', padding: '12px 16px', border: '2px solid var(--border)' }}
              />
            </div>
            {!isIn(selected) ? (
              <button onClick={() => handleAction('in')} disabled={loading} className="ripple-btn"
                style={{
              ...
              <button onClick={() => handleAction('out')} disabled={loading} className="ripple-btn"
                style={{
                  width: '100%', padding: '22px', borderRadius: 16,
